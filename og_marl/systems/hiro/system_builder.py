...
from og_marl.systems.hiro.executor import HIROExecutor
from og_marl.systems.hiro.trainer import HIROTrainer
from og_marl.utils.executor_utils import concat_agent_id_to_obs



class HIROSystemBuilder(SystemBuilderBase):
    """Implementation of ITD3 and MATD3 systems."""
    ...
    def __init__(
        ...
        q_optimizer=snt.optimizers.Adam(learning_rate=1e-4),
        ...
        self._hi_critic_network = hi_critic_network
        self._lo_q_network = lo_q_network

        # Optimisers
        self._hi_policy_optimizer = policy_optimizer
        self._hi_critic_optimizer = critic_optimizer
        ...
        self._add_agent_id_to_obs = False
        ...
        self._executor_fn = HIROExecutor
    ...
    def _build_executor(
        ...
        executor = self._executor_fn(
            ...
            hi_policy_network=policy_network,
            lo_q_network=q_network,
            adder=adder,
    ...
    def _build_trainer(self, dataset, logger, gaussian_noise_network):
        ...
        specs = env_spec.get_agent_specs()
        # agent_act_spec = list(specs.values())[0].actions  # assume all the same spec
        agent_obs_spec = list(specs.values())[0].observations
        hi_act_spec = dm_env.specs.BoundedArray(agent_obs_spec.observation.shape, np.float32, minimum=-1, maximum=1)
        gaussian_noise_network = snt.Sequential(
            [
                mava_networks.ClippedGaussian(self._trainer_sigma),
                mava_networks.ClipToSpec(hi_act_spec),
            ]
        )
        ...
        trainer = self._trainer_fn(
            ...
            hi_critic_network=hi_critic_network,
            lo_q_network=lo_q_network,
            hi_policy_optimizer=self._hi_policy_optimizer,
            hi_critic_optimizer=self._hi_critic_optimizer,
            lo_q_optimizer=self._q_optimizer,
    ...
    def _initialise_networks(self):
        hi_policy_network = copy.deepcopy(self._hi_policy_network)
        hi_critic_network = copy.deepcopy(self._hi_critic_network)

        lo_q_network = copy.deepcopy(self._lo_q_network)
        ...
        dummy_hi_action = tf.identity(dummy_observation)
        # Initialise critic network
        hi_critic_network.initialise(dummy_observation, dummy_hi_action)

        # Initialize policy and critic network variables
        hi_policy_network(dummy_observation, hi_policy_network.initial_state(1))

        dummy_lo_inputs = tf.concat([dummy_observation, dummy_hi_action], axis=-1)
        lo_q_network(dummy_lo_inputs, lo_q_network.initial_state(1))
